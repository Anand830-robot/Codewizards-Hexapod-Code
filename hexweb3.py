...
# ---------- STOP-ALL GATE ----------
STOP_ALL = False  # when True, ignore all future servo angle commands

# Keep original low-level servo write
_orig_set_servo_angle = state.ctrl.servo.set_servo_angle

def guarded_set_servo_angle(channel, angle):
    """Ignore servo commands when STOP_ALL is True."""
    if STOP_ALL:
        return
    _orig_set_servo_angle(channel, angle)

# Monkey-patch the servo instance inside ControlNoPoint
...
def handle_pan_tilt(cmd: str, step: int):
    ...
    with lock:
        ...
        if cmd == "center":
            ...
            # Relax ALL servos (pan/tilt + legs) and engage STOP_ALL gate
            STOP_ALL = True
            c.servo.relax()
            state.pan_angle = pan
...
def cmd_route():
    global STOP_ALL
    # Any hex command clears STOP so movement can resume
    STOP_ALL = False
...
def stopall_route():
    global STOP_ALL
    with lock:
        STOP_ALL = True           # block all future set_servo_angle calls
        state.ctrl.servo.relax()  # cut power once right now
    return jsonify({"ok": True, "message": "All servos relaxed (legs + pan/tilt)"})

@app.post("/pt")
def pt_route():
    global STOP_ALL
    # Any pan/tilt command (except internal relax call we already handled)
    # should clear STOP so camera/legs can move again
