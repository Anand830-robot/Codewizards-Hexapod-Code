...
def led_blink(r, g, b, times=3, on_t=0.2, off_t=0.15):
    def _blink():
        for _ in range(times):
            if STOP_ALL:  # let stop-all break out
                break
            led_solid(r, g, b)
            time.sleep(on_t)
            led_off()
...
def led_breathe(r, g, b, cycles=2):
    def _breathe():
        for _ in range(cycles):
            if STOP_ALL:
                break
            for lvl in list(range(0, 21)) + list(range(20, -1, -1)):
                if STOP_ALL:
                    break
                scale = lvl / 20.0
    ...
    def _run():
        try:
            ...
            if name == "demo1":
                ...
                set_cmd("raise")
                time.sleep(0.6)
                # 4 forward steps with pan sweep
                for step in range(4):
                    if STOP_ALL:
                        break
                    # step forward
                ...
                # center pan, reset pose, soft fade LED
...
function refreshSensors(){
  fetch('/sensors')
    ...
    .then(j => {
      if(j.ok){
        if(typeof j.battery === 'number' && !isNaN(j.battery)){
          document.getElementById('bat_val').textContent = j.battery.toFixed(2);
        }
        if(typeof j.distance === 'number' && !isNaN(j.distance)){
...
def sensors_route():
    ...
    try:
        with lock:
            try:
                bat_val = float(state.adc.read_battery_voltage())
            except Exception as e:
                err_msg = f"Battery read error: {e}"

            try:
                dist_val = float(state.ultrasonic.get_distance())
            except Exception as e:
                err_msg = (err_msg or "") + f" Ultrasonic read error: {e}"
    except Exception as e:
        # outer error
        return jsonify({"ok": False, "error": f"sensors failure: {e}"}), 200

    if err_msg:
        # still return any partial values we have
        return jsonify({"ok": False, "battery": bat_val, "distance": dist_val, "error": err_msg}), 200
